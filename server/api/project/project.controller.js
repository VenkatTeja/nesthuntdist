...
function create(req, res) {
  ...
  if (body.type.type == 1) delete body.type['bSize'];else if (body.type.type == 2) delete body.type['lSize'];
  ...
  return _project2.default.ProjectType.create(body.type).then(function (projectType) {
    ...
    newProject.images = ['https://' + process.env.BUCKET + '.cellar.services.clever-cloud.com/imagesInt/' + newProject._id + '_1.jpg', 'https://' + process.env.BUCKET + '.cellar.services.clever-cloud.com/imagesInt/' + newProject._id + '_2.jpg', 'https://' + process.env.BUCKET + '.cellar.services.clever-cloud.com/imagesExt/' + newProject._id + '_1.jpg', 'https://' + process.env.BUCKET + '.cellar.services.clever-cloud.com/imagesExt/' + newProject._id + '_2.jpg'];
...
var addjson = function addjson(i, prj, res) {
  _user2.default.find({ email: prj[i].mailId.toLowerCase() }).exec().then(function (user) {
    if (!user.length) {
      ...
      newUser.save().then(function (user) {
        ...
        if (budget.length == 1) budget.push(budget[0]);
        var status;
        var type = 2;
        if (prj[i].progress == "Completed") status = 0;else if (prj[i].progress == "Under Construction") status = 1;else if (prj[i].progress == "Vacant land") {
          type = 1;
          status = 0;
        }
        var body = {
        ...
        if (prj[i]['Number of 1 BHKunits'] != 'NA') {
          ...
          if (size[1]) size[1] = size[1].split(' ')[0];else {
        ...
        console.log(size, i);
        if (type == 1) body.type.lSize = { min: size[0], max: size[1] };else body.type.bSize = { min: size[0], max: size[1] };

        _project2.default.ProjectType.create(body.type).then(function (projectType) {
          body.type = projectType._id;
          var newProject = new _project2.default.Project(body);
          ...
          newProject.save().then(function (project) {
            ...
            if (i + 1 < prj.length) addjson(i + 1, prj, res);
          }).catch(handleError(res));
        }).catch(handleError(res));
      });
    } else {
    ...
    _project2.default.ProjectType.findById(req.body.data.type._id).exec().then(function (projectType) {
      ...
      projectType.save().then(function (projectType) {
        for (var key in req.body.data) {
          console.log(key);
          if (req.body.data.hasOwnProperty(key)) if (key != 'type') {
            console.log(req.body.data[key]);
            project[key] = req.body.data[key];
          }
        }
        console.log(project);
        project.save().then(respondWithResult(res)).catch(handleError(res));
      }).catch(handleError(res));
    });
  }).catch(handleError(res));
}
